...
//
// METAR source is from: http://weather.noaa.gov/pub/data/observations/metar/stations/EGDM.TXT
// in this example the station requested is EGDM (UK, Boscombe Down)
/* The typical format of the response received is:
 2013/01/28 21:50
 EGDM 282150Z AUTO 24015KT 9999 BKN130/// 10/07 Q1001
 
*/
////////////////////////////////////////////////////////////////////////////////////
String version_num = "METAR ESP Version 1.01b";
#include <ESP8266WiFi.h>
#include "SPI.h"
#include "Adafruit_GFX.h"
#include "Adafruit_ILI9341.h"

#define _CS   15 // D8 on ESP
#define _DC    2 // D4 on ESP
#define _mosi 13 // D7 on ESP
#define _sclk 14 // D5 on ESP
#define _rst  01
#define _miso 01
...
void loop(){
  GET_METAR("EGDM", "1 EGDM Boscombe Down");
  GET_METAR("EGVN", "2 EGVN Brize Norton");
  GET_METAR("EGGD", "3 EGGD Bristol/Lulsgate");
  GET_METAR("EGDY", "4 EGDY Yeovilton");
  GET_METAR("EGCC", "5 EGCC Manchester Airport");
  GET_METAR("EGHQ", "6 EGHQ Newquay");
  GET_METAR("EGSS", "7 EGSS Stansted");
} 
...
void GET_METAR(String station, String Name) { //client function to send/receive GET request data.
  String  metar = " ";
  String  metar_line = " ";
  boolean metar_record_found = false;
  boolean line_one_done      = false;
  ...
  if (client.connect("weather.noaa.gov",80)) {  //starts client connection, checks for connection
    display_item(265,230,"Connected",RED,1);
    client.println("GET /pub/data/observations/metar/stations/" + station + ".TXT HTTP/1.0"); //download text
    client.println("Host: weather.noaa.gov");
    ...
    /* Returns the following from the server_address, so look for the blank line, then the METAR
    HTTP/1.0 200 OK
    Last-Modified: Tue, 26 Feb 2013 22:08:11 GMT
    ETag: "cac59b-49-4d6a7e4a8e8c0"
    Server: Apache
    Accept-Ranges: bytes
    Content-Length: 73
    Content-Type: text/plain; charset=UTF-8
    Cache-Control: max-age=158
    Expires: Tue, 26 Feb 2013 22:53:09 GMT
    Date: Tue, 26 Feb 2013 22:50:31 GMT
    Connection: close\r\n
    \r\n
    2013/02/26 21:50
    EGDM 262150Z AUTO 04009KT 5000 HZ OVC005/// 03/02 Q1034
    */
//  while(client.connected()  && !client.available() ) {delay(1); } //must wait for data
  while (client.connected() /*&&  client.available()*/ ) { //connected or data available
    char c = client.read(); //gets byte from ethernet buffer
    if ((c != '\n') || (c != '\r')) { 
      metar_line = metar_line + c;
    } 
    if (metar_record_found && c == '\n' && metar_line.length() >= 14) {
      if (line_one_done) { 
        metar = metar_line; 
      } 
      else {
    ...
    if (metar_line == "\r") {
      metar_record_found = true; 
      metar_line = "";
    } // A blank line starts the METAR Record body text
    if (c == '\n') {
      metar_line = "";
    }
  } 
...
void display_metar(String metar) {
  ...
  while (parameter != NULL) {
    display_item(0,0, String(parameter),RED,2); // Station Name
    temp_strA = strtok(NULL," ");
    display_item(70,00, "(" + temp_strA.substring(0,2) + ") " + temp_strA.substring(2,6)+"Hr",GREEN,2); // Date-time 

//----------------------------------------------------------------------------------------------------
// Process any reported station type e.g. AUTO means Automatic
    temp_strA = strtok(NULL," ");
    if (temp_strA == "AUTO") {
      display_item(54,0,"A",CYAN,2);
      temp_strA = strtok(NULL," ");
    }
...
// Process any reported visibility e.g. 6000 means 6000 Metres of visibility
    ...
    if (temp_strA == "CAVOK") {
      ...
      temp_strB = temp_strA + " Mts of visibility"; 
      }
    }
...
// Process any reported weather conditions e.g. -RA means light rain
    ...
    if ((temp_strA == "//" || conditions_test.indexOf(conditions_start) >= 0)
      ...
      if (temp_strA.length() == 2) { display_item(136,175,temp_strB + display_conditions(temp_strA),WHITE,1);}
      else {
        display_item(136,175,temp_strB + display_conditions(temp_strA.substring(0,2)),WHITE,1); 
...
// Process any reported weather conditions e.g. -RA means light rain
    ...
    if ((temp_strA == "//" || conditions_test.indexOf(conditions_start) >= 0)
      ...
      if (temp_strA.length() == 2) { display_item(230,175,temp_strB + display_conditions(temp_strA),WHITE,1);}
      else {
        if (temp_strA.length() >= 5) { // sometimes up to three cateries are reported
          display_item(230,175,"Poor Vert Vis.",WHITE,1);
        }
        else {
          display_item(230,175,temp_strB + display_conditions(temp_strA.substring(0,2)),WHITE,1); 
        }
      }
      parameter = strtok(NULL," ");
      temp_strA = parameter;
...
// Now process up to two secondary weather reports
   ...
   Process_secondary_reports(temp_strA,138);
   temp_strA = strtok(NULL," "); // Get last tokens, can be a secondary weather report
   Process_secondary_reports(temp_strA,148);
...
String convert_clouds(String source) {
  ...
  if (height.length() <= 2 ) {
    if (height.length() <= 1) height = " " + height;
  }
  ...
  if (cloud !="SKC" && cloud != "CLR") {height = " at " + height + "00ft";} else height = "";
  ...
  if (cloud == "NSC")    {return "No signficiant clouds" + height;}
...
void display_status() {
  ...
  delay(1000);
  tft.println(" Done");
  tft.println("Waiting for IP address...");
